...
public class MentionHere implements IMention {
  private final MentionTimeout timer;
  private final String prefix;
  private final String permission;

  public MentionHere(String permission, String prefix) {
  ...
  public boolean execute(Habbo sender, String receiver, String message) {
    if (!receiver.equalsIgnoreCase("here")) return false;

    if (Variables.SANITIZE) message = sanitize(message);

    message = getReplacedMessage(sender, message);
    THashMap<String, String> alert =
        bubbleAlert(sender, message, getReplaceLook(sender), isFollowEnabled());
    sendMention(alert);
    return true;
  }

  private static void sendMention(THashMap<String, String> alert) {
    for (Habbo habbo : Emulator.getGameEnvironment().getHabboManager().getOnlineHabbos().values()) {
      if (habbo == null
          || habbo.getHabboStats().blockStaffAlerts
          || habbo.getRoomUnit() == null
          || habbo.getRoomUnit().isIdle()) continue;
  ...
  private static String getReplaceLook(Habbo sender) {
